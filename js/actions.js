	...
	*/
	...
	$("#profile_form").on("submit", function (event) {
		...
		$.ajax({
			url: "action.php",
			method: "POST",
			data: $("#profile_form").serialize() + "&update_profile=1",
			success: function (data) {
				$(".overlay").hide();
				if (data.trim() == "update_success") {
					$("#profile_msg").html("<div class='alert alert-success'>Profile Updated Successfully. Page will reload.</div>");
					setTimeout(function () {
						window.location.reload();
					}, 2000);
				} else {
					$("#profile_msg").html("<div class='alert alert-danger'>" + data + "</div>");
				}
			}
		})
	})
